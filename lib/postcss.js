...
var Replace = function Replace(output) {
	_classCallCheck(this, Replace);

	console.log("Replace", output);
	var path = (0, _common.fst)(output);
	if (shell.test("-d", path)) throw new Error("Not implemented by postcss-cli");
};

var Directory = function Directory(output) {
...
function getTypeFromOption(parameters) {
 ...
 .filter(identity)*/
	...
	/* Third: composed FP */
	var getCallType = (0, _composer.compose)(searchWith, callWith(getPathFromParameters, parameters), callWith(convertPathsToObject, typeHandlers), function (x) {
		return Array.prototype.filter.call(x, _common.identity);
	});
	var CallType = getCallType(types, alternatives);
...
function convertPathsToObject(classTuple, paths) {
	return paths.map(function (path, i) {
		return (0, _common.isNotEmpty)(path) && classTuple[i].bind(null, (0, _common.fst)(path));
	});
}
/* zipWith :: (a -> b -> c) -> [a] -> [b] -> [c] */
