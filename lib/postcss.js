...
function callPath(args) {
  ...
  var callTypes = zip(types, alternatives).map(function (flags) {
    return (0, _common.fst)(flags.map(function (flag) {
      return (0, _common.getParameters)(flag, args);
    })
    //	.filter( identity )
    );
  }
  //	.filter( identity )
  ).map(function (path, i, all) {
    return path.length ? new typeHandlers[i]((0, _common.fst)(path)) : null;
  });
