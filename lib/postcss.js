...
function getTypeFromOption(parameters) {
 ...
 .filter(identity)*/
	...
	//return CallType
	var searchFlags = zip(types, alternatives);
	var output = (0, _common.isEmpty)(searchFlags.map(bind(_common.getParameters, parameters)));
	var j = map(bind(_common.getParameters, parameters))(searchFlags);
...
/** tco */
function foldl(fn, terminalValue, _ref) {
